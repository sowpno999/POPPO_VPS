...
import shutil
import zipfile
import py7zr #not available in some site
import psutil
import json
import hashlib
import secrets
...
#ngrok.set_auth_token("3EnpYvnV82WwYDCqCQvLPBDreJI_4Snii6Um7vzbqwVwZpPLj")
...
DEFAULT_CONFIG = {
    "site_title": "POPPO PREMIUM VPS",
    "site_header": "POPPO VPS",
    ...
    "passwords": {
        "secret": hashlib.sha256("FFBY7X".encode()).hexdigest(),
...
def telegram_bot():
    try:
        ...
        # Updated bot script with pyTelegramBotAPI (safe formatting)
...
def send_api_response(message):
    ...
    try:
        r = requests.get(url, timeout=15, headers={{'User-Agent': 'POPPO-TeleBot'}})
...
#    _t = ngrok.connect(addr=8080, proto="http")
    ...
    print("=" * 50)
    print("POPPO PREMIUM VPS - Starting...")
    print("=" * 50)
    print(f"Port: {port}")
#    print(f"URL: {_t.public_url}")
    print(f"Debug: {debug}")
    ...
    print("  Secret: FFBY7X (Can change user password)")
